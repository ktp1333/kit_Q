...
app.controller("q_screenController", function ($scope, $location, $http, $timeout, $mdSidenav, $window, $q, $interval, $filter, globalSetting) {
  var vm = this;
  // vm.department = "จุดคัดกรองชั้น M";
  // vm.department =  "จุดคัดกรองผู้ป่วยใหม่จักษุ";
  ...
  function listdep() {
    $http.post('/local_host/finddepartment', {
      ...
      for (let index = 0; index < $scope.department.length; index++) {
        ...
        //   $scope.department[index].show = false;
        //           } else if ($scope.department[index].department == "จุดเวชระเบียนชั้น 1") {
        //   $scope.department[index].show = false;
        } else {}
      }
  ...
  function updatestatus_call(statusscreen, patient) {


    if (statusscreen == 'คิวเรียก') {
  ...
  function updatestatus_finish(statusscreen, patient) {
    vm.finish = true;
    vm.statusscreen = statusscreen;
    vm.pt = patient;

  }

  function updatestatus(statusscreen, patient) {
    ...
    $http
      ...
      .success(function (data) {
        vm.thisstatusscreen = statusscreen;
        ...
        $http
          ...
          .success(function (data) {
            if (data && data.data.length > 0) {
              ...
              vm.mwait = 0;
              for (let index = 0; index < doc1.length; index++) {
                switch (doc1[index].statusscreen) {
                  case 'รอเรียกคิว':
                    vm.mwait = vm.mwait + 1;
                    break;
                  case 'พักคิว':
  ...
  function save_rec(department, patient, statusscreen) {
