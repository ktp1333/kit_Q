...
app.controller("q_cashierController", function ($scope, $location, $http, $timeout, $mdSidenav, $window, $q, $interval, $filter, globalSetting) {
  var vm = this;
  ...
  function updatestatus_call(statuscx, patient) {
    if (statuscx == 'คิวเรียก') {
  ...
  function updatestatus_finish(statuscx, patient) {
    vm.finish = true;
    vm.statuscx = statuscx;
    vm.pt = patient;
    $scope.selectopd = patient;
  }

  function updatestatus(statuscx, patient) {
    console.log(statuscx);
    console.log(patient);
    if (statuscx != 'คิวเรียก') {
      vm.callq = false;
    }
        ...
        $http
          ...
          .success(function (data) {
            if (data && data.data.length > 0) {
              ...
              vm.mwait = 0;
              for (let index = 0; index < doc1.length; index++) {
                switch (doc1[index].currenttracking.statuscx) {
                  case 'รอเรียกคิว':
                    vm.mwait = vm.mwait + 1;
                    break;
                  case 'พักคิว':
  ...
  function save_rec(department, patient, statuscx) {
